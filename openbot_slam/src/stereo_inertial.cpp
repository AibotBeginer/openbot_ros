...
#include "openbot_slam/stereo_inertial.hpp"
#include "openbot_slam/utility.hpp"

namespace openbot_ros {

StereoInertial::StereoInertial(ORB_SLAM3::System* pSLAM, bool DoRectify, bool DoEqual)
    : Node("stereo_inertial_node")
{
    ...
    // 获取参数值
    this->get_parameter("vocabulary_file", vocabulary_file_);
    this->get_parameter("yaml_config_file", yaml_config_file_);

    // 打印参数值
    RCLCPP_INFO(this->get_logger(), "vocabulary_file: %s", vocabulary_file_.c_str());
    RCLCPP_INFO(this->get_logger(), "yaml_config_file: %s", yaml_config_file_.c_str());
    ...
    if (doRectify_)
    {
        ...
        cv::FileStorage fsSettings(yaml_config_file_, cv::FileStorage::READ);
    ...
    subImu_ = this->create_subscription<ImuMsg>("imu", 1000, std::bind(&StereoInertial::GrabImu, this,  std::placeholders::_1));
    subImgLeft_ = this->create_subscription<ImageMsg>("camera/left", 100, 
        std::bind(&StereoInertial::GrabImageLeft, this,  std::placeholders::_1));
    subImgRight_ = this->create_subscription<ImageMsg>("camera/right", 100, 
        std::bind(&StereoInertial::GrabImageRight, this,  std::placeholders::_1));

    syncThread_ = new std::thread(&StereoInertial::SyncWithImu, this);
}

StereoInertial::~StereoInertial()
{
    // Delete sync thread
    syncThread_->join();
    delete syncThread_;
...
void StereoInertial::GrabImu(const ImuMsg::SharedPtr msg)
{
    bufMutex_.lock();
    imuBuf_.push(msg);
    bufMutex_.unlock();
...
void StereoInertial::GrabImageLeft(const ImageMsg::SharedPtr msgLeft)
{
    bufMutexLeft_.lock();
...
void StereoInertial::GrabImageRight(const ImageMsg::SharedPtr msgRight)
{
    bufMutexRight_.lock();
...
cv::Mat StereoInertial::GetImage(const ImageMsg::SharedPtr msg)
{
