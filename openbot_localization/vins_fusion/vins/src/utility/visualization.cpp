...
void pubTF(Estimator &estimator, const std_msgs::msg::Header &header)
{
    if (estimator.solver_flag!= Estimator::SolverFlag::NON_LINEAR)
        return;

    if (broadcaster == nullptr) {
        broadcaster = std::make_shared<tf2_ros::TransformBroadcaster>(*estimator.node());
    }
...
//     q.setZ(correct_q.z());
//     transform.setRotation(q);
//     // br->sendTransform(tf2::StampedTransform(transform, header.stamp, "map", "base_link"));
//     br->sendTransform(tf2::StampedTransform(transform, header.stamp, "map", "base_link"));
...
//     transform.setRotation(q);
//     // br->sendTransform(tf2::StampedTransform(transform, header.stamp, "base_link", "camera"));
//     br->sendTransform(tf2::StampedTransform(transform, header.stamp, "base_link", "camera"));
