...
int main(int argc, char **argv)
{
	...
	auto n = rclcpp::Node::make_shared("vins_node");

    std::string config_file;
    n->declare_parameter("config_filename", "");
    config_file = n->get_parameter("config_filename").as_string();
    RCLCPP_INFO(n->get_logger(), "config_filename: %s", config_file.c_str());

    config_file = ament_index_cpp::get_package_share_directory("vins") + "/config/" + config_file;
    RCLCPP_INFO(n->get_logger(), "config filename: %s", config_file.c_str());

    readParameters(config_file);
    estimator.setParameter();
    estimator.set_node(n);
    ...
    auto sub_feature = n->create_subscription<sensor_msgs::msg::PointCloud>("/feature_tracker/feature", qos_profile, feature_callback);
    auto sub_img0 = n->create_subscription<sensor_msgs::msg::Image>(IMAGE0_TOPIC, qos_profile, img0_callback);
    
    rclcpp::Subscription<sensor_msgs::msg::Image>::SharedPtr sub_img1 = NULL;
    if(STEREO)
    {
        sub_img1 = n->create_subscription<sensor_msgs::msg::Image>(IMAGE1_TOPIC, qos_profile, img1_callback);
    }
    
    auto sub_restart = n->create_subscription<std_msgs::msg::Bool>("/vins_restart", qos_profile, restart_callback);
    auto sub_imu_switch = n->create_subscription<std_msgs::msg::Bool>("/vins_imu_switch", qos_profile, imu_switch_callback);
    auto sub_cam_switch = n->create_subscription<std_msgs::msg::Bool>("/vins_cam_switch", qos_profile, cam_switch_callback);
